 ...
 * @since 2023-07-25
 */
public class GetChargesXMLPayload {

    /**
     * Сформировать Payload для GetCharges для вставки в элемент XML
     *
     * @param getChargesRequest              Объект GetChargesRequest
     ...
     * @since 2023-07-25
     */
    public OMElement formPayload(GetChargesRequest getChargesRequest, CheckPaymentRequisitesResponse checkPaymentRequisitesResponse) throws AbstractException {
        ...
        try {
            ...
            // List all GetChargesRequest fields and add them to the response
            List<GetChargesRequest.Field> fields = getChargesRequest.getFields();
            for (GetChargesRequest.Field field : fields) {
                Element idElement = document.createElement(field.getFieldName().toLowerCase());
                Text idText = document.createTextNode(field.getFieldValue());
            ...
            throw new AbstractException("Не удалось сформировать ответ для GetCharges");
        }
    }
}
